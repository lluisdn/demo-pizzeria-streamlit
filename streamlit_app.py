...
# ==========================================
st.subheader("2. Corbes de Demanda vs. Capacitat (Horari: 9h-14h i 17h-21h)")
...
hores = ["09:00", "10:00", "11:00", "12:00", "13:00", "14:00 (Tancat)", "17:00", "18:00", "19:00", "20:00", "21:00"]

if dia_seleccionat == "Divendres":
    # Matí estable, pico a les 13h-14h. Tarda pico molt fort a les 19h-20:30h
    escalivada      = [5,  8, 12, 18, 25, 0, 10, 15, 25, 30, 15]
    carbasso        = [4,  6, 10, 14, 20, 0,  8, 12, 20, 24, 10]
    ceba            = [6, 10, 15, 22, 30, 0, 12, 18, 30, 35, 18]
    empanada        = [3,  5,  8, 12, 18, 0,  6, 10, 16, 20,  8]
    alberginia_mel  = [4,  6, 10, 15, 22, 0,  8, 12, 22, 26, 12]
    ceba_bolets     = [3,  5,  9, 14, 20, 0,  7, 11, 20, 25, 10]
else:
    # Dissabte: Pics alts a migdia (compres de cap de setmana) i a la tarda
    escalivada      = [8, 12, 18, 25, 32, 0, 12, 18, 28, 32, 18]
    carbasso        = [6, 10, 14, 20, 26, 0, 10, 14, 22, 26, 12]
    ceba            = [9, 15, 22, 30, 38, 0, 15, 22, 34, 38, 20]
    empanada        = [5,  8, 12, 16, 22, 0,  8, 12, 18, 22, 10]
    alberginia_mel  = [6, 10, 15, 22, 28, 0, 10, 15, 24, 28, 14]
    ceba_bolets     = [5,  9, 14, 20, 25, 0,  9, 14, 22, 25, 12]

# Capacitat màxima ajustada a 90 porcions per hora (0 quan està tancat)
capacitat_maxima = [90 if h != "14:00 (Tancat)" else 0 for h in hores]
...
st.error("⚠️ **Punt Crític Operatiu:** En les franges de 12:30 a 14:00 i de 19:00 a 20:30 la demanda frega o supera la capacitat de 90 porcions/hora del forn, generant cues al carrer i pèrdua de clients que no volen esperar.")
...
with col1:
    st.markdown("### 🌐 1. Nova Web i Comandes En Línia (Takeaway)")
    st.write(
    ...
    st.markdown("### 🍰 2. Promoció de Dolços, Pastissos i Reposteria")
    st.write(
        "**Oportunitat:** Introduir o potenciar la gamma de producte dolç per augmentar el tiquet mitjà per client. "
        "Són productes d'alta rotació al matí (9h-11h) i a mitja tarda, aprofitant hores en què el forn té capacitat lliure."
    )
